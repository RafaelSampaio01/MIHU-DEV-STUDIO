...
// ================================
// WebSerial MicroPython (USB) - PERFEITO
// ================================
class WebSerialMP {
  constructor() {
    this.port = null;
  ...
  async connect({ baudRate, request = true } = {}) {
    ...
    if (request) {
      ...
      if (!ports.length) throw new Error("Nenhuma porta autorizada.");
      this.port = ports[0];
    }
  ...
  async ctrlC() { await this.writeBytes([0x03]); await sleep(20); }
  async ctrlD() { await this.writeBytes([0x04]); await sleep(20); }
  async ctrlA() { await this.writeBytes([0x01]); await sleep(20); }
  async ctrlB() { await this.writeBytes([0x02]); await sleep(20); }

  async ensureFriendly() {
    try {
      await this.ctrlB();
      await sleep(60);
    } catch {}
  ...
  async rawExec(code, { endMarker = "___END___", timeoutMs = 12000 } = {}) {
    if (this.busy) throw new Error("Serial ocupada");
    this.busy = true;
    ...
    try {
      ...
      await this.ctrlA();
      await this.waitFor("raw REPL; CTRL-B to exit", 3000);
...
class WebBLETransport {
  constructor() {
    ...
    this.server = null;
    this.service = null;
    this.txChar = null; // Característica de escrita (para enviar para a placa)
    this.rxChar = null; // Característica de notificação (para receber da placa)
    this.rx = "";
    this.abortRequested = false;
    this.busy = false;
    this.mtu = 20;
    
    // Constantes do Nordic UART Service
    this.NUS_SERVICE = "6e400001-b5a3-f393-e0a9-e50e24dcca9e";
    this.NUS_TX = "6e400002-b5a3-f393-e0a9-e50e24dcca9e"; // Write
    this.NUS_RX = "6e400003-b5a3-f393-e0a9-e50e24dcca9e"; // Notify
  }
  ...
  async connect() {
    try {
      console.log("Procurando dispositivos BLE...");
      ...
      this.service = await this.server.getPrimaryService(this.NUS_SERVICE);
      console.log("Serviço NUS encontrado");

      // Característica de escrita (TX do ponto de vista do dispositivo)
      this.txChar = await this.service.getCharacteristic(this.NUS_TX);
      
      // Característica de notificação (RX do ponto de vista do dispositivo)
      this.rxChar = await this.service.getCharacteristic(this.NUS_RX);
      ...
      console.log("BLE conectado com sucesso");
  ...
  async disconnect() {
    try {
      ...
      console.error("Erro ao desconectar BLE:", error);
    } finally {
      this.device = null;
      this.server = null;
      this.service = null;
  ...
  async writeBytes(bytes) {
    if (!this.txChar) throw new Error("BLE não conectado");
    
    const data = new Uint8Array(bytes);

    for (let i = 0; i < data.length; i += this.mtu) {
      const chunk = data.slice(i, i + this.mtu);
      try {
        await this.txChar.writeValue(chunk);
        await sleep(10); // Pequena pausa entre chunks
      } catch (err) {
        console.error("Erro ao escrever chunk BLE:", err);
        throw err;
      }
    }
  }
  ...
  async writeText(text) {
    ...
    await this.writeBytes(enc.encode(text));
  }

  async ctrlC() { await this.writeBytes([0x03]); await sleep(30); }
  async ctrlD() { await this.writeBytes([0x04]); await sleep(30); }
  async ctrlA() { await this.writeBytes([0x01]); await sleep(30); }
  async ctrlB() { await this.writeBytes([0x02]); await sleep(30); }
  ...
  async rawExec(code, { endMarker = "___END___", timeoutMs = 20000 } = {}) {
    if (this.busy) throw new Error("BLE ocupado");
    this.busy = true;
    ...
    try {
      ...
      await this.ctrlA();
      await this.waitFor("raw REPL; CTRL-B to exit", 4000);
...
const usbDev = new WebSerialMP();
const bleDev = new WebBLETransport();
let dev = usbDev; // Dispositivo ativo (começa com USB)
...
async function deviceWriteText(path, text) {
  ...
  if (bytes.length > 80000) throw new Error("Arquivo grande demais (limite ~60KB).");
...
`;
  ...
  try {
    await dev.rawExec(py, { endMarker: "___END___", timeoutMs: 80000 });
  } catch (e) {
...
function tourBuildSteps(){
  return [
    ...
    { sel:null, place:"center", title:"Pronto!",
      body:`Agora você já pode programar seu ESP32! 🚀`,
...
function tourOpenUI(){
  ...
  ui.tourBackdrop.classList.remove("hidden");
  ui.tourHL.classList.remove("hidden");
...
function tourCloseUI(){
  ...
  ui.tourBackdrop.classList.add("hidden");
  ui.tourHL.classList.add("hidden");
  ui.tour.setAttribute("aria-hidden","true");
  ui.tourHL.style.cssText = "";
  ui.tour.style.cssText = "";
  tourOpen = false;
}
...
function tourShowStep(i){
  ...
  if (!step.sel || step.place==="center") {
    ...
    ui.tour.style.left = "50%";
    ui.tour.style.top = "50%";
    ui.tour.style.transform = "translate(-50%, -50%)";
    return;
  }
...
function tourPositionToElement(el, place){
  ...
  const cardW = ui.tour.offsetWidth || 420;
  const cardH = ui.tour.offsetHeight || 220;
  ...
  ui.tour.style.left = cx + "px";
  ui.tour.style.top = cy + "px";
  ui.tour.style.transform = "none";
}
...
function setConnectButtonState(connected) {
  ui.btnConnect.textContent = connected ? "USB Desconectar" : "USB Conectar";
...
ui.btnConnect.addEventListener("click", async () => {
  ...
  try {
    ...
    // Se BLE estiver conectado, desconecta
    if (bleDev.connected) {
      await bleDev.disconnect();
    }
    
    await usbDev.connect({ baudRate, request: true });
    dev = usbDev;
...
bleBtn.addEventListener("click", async () => {
  try {
    // Se BLE já estiver conectado → desconecta
    if (bleDev.connected) {
      await bleDev.disconnect();
      
      bleBtn.textContent = "BLE Conectar";
      bleBtn.classList.remove("danger", "connected");
      bleBtn.classList.add("primary");

      setStatus(false, "BLE desconectado");
      setConnectedUI(false);
      
      // Se USB estiver disponível, volta para ele
      if (usbDev.connected) {
        dev = usbDev;
        setStatus(true, "USB conectado");
        setConnectedUI(true);
      }
    ...
    // Se USB estiver conectado, desconecta primeiro
    if (usbDev.connected) {
      await usbDev.disconnect();
    }

    bleBtn.textContent = "Conectando...";
    bleBtn.disabled = true;
    bleBtn.classList.add("connecting");

    await bleDev.connect();

    // Torna BLE o dispositivo ativo
    dev = bleDev;

    bleBtn.textContent = "BLE Desconectar";
    bleBtn.classList.remove("primary", "connecting");
    bleBtn.classList.add("danger", "connected");
    bleBtn.disabled = false;

    setStatus(true, "BLE conectado");
    setConnectedUI(true);

    await bleDev.syncRepl();
    await refreshFiles("/");

  } catch (e) {
    console.error("Erro BLE:", e);
    bleBtn.textContent = "BLE Conectar";
    bleBtn.classList.remove("danger", "connected", "connecting");
    bleBtn.classList.add("primary");
    bleBtn.disabled = false;
    
    alert("Erro BLE: " + (e.message || e));
  }
});
...
window.addEventListener("keydown", (e) => {
  ...
  if (ctrlOrCmd && e.key === "Enter") { 
    e.preventDefault(); 
    if (!ui.btnRun.disabled) runEditor(); 
  }
  
  if (ctrlOrCmd && (e.key === "s" || e.key === "S")) { 
    e.preventDefault(); 
    if (!ui.btnSaveToDevice.disabled) saveToDevice(); 
  }
